// Copyright (C) Scott Dwyer and OpenSuperClone contributors.
// You may use/distribute/modify this freely, under the terms of
// the GNU General Public License version 2 or later version.
// This software is distributed WITHOUT ANY WARRANTY.

#ifndef OPENSUPERCLONE_H
#define OPENSUPERCLONE_H

#define DEFAULT_SECTOR_SIZE 512
#define ADVANCED_SECTOR_SIZE 4096
#define CRASH 256
#define TRANSLATE 257
#define ENABLE_REBUILD_ASSIST 258
#define ENABLE_HBA_RESET 265
#define REBUILD_ASSIST_TEST 269
#define ENABLE_HBA_TEST 270
#define DISABLE_AHCI_IDENTIFY 271

#define SUPERTOOL 263
#define ATAMODE 512
#define SCSIMODE 513
#define TESTSKIP 514
#define TESTSKIPFAST 515
#define TESTSKIPMB 516
#define USB1VENDORID 517
#define USB1PRODUCTID 518
#define DEBUGOUT 519
#define VERBOSEOUT 520
#define PORT 521

#define NONTRIED 0
#define NONTRIMMED 0x10
#define NONDIVIDED 0x20
#define NONSCRAPED 0x30
#define BAD 0x40
#define BAD_HEAD 0x80
#define FINISHED 0x7f
#define PHASE1 0x0
#define PHASE2 0x2
#define PHASE3 0x6
#define PHASE4 0x8
#define TRIMMING 0x10
#define DIVIDING1 0x20
#define DIVIDING2 0x22
#define SCRAPING 0x30
#define RETRYING 0x40
#define REBUILDASSIST 0X80
#define FILLING 0x100
#define STATUS_MASK 0xff
#define BAD_HEAD_STATUS 0x80
#define EXPORT_STATUS_MASK 0x70
#define HALF_MASK 0xffffffff
#define INFO_MASK 0xffffff00
#define SKIP_INFO_MASK 0xff00
#define SKIP_RESET_MASK 0xffffffffffff00ff
#define ADD_STATUS_MASK 0xffffffffffffff00
#define FULL_MASK 0xffffffffffffffff
#define BAD_HEAD_MASK 0xffffffffffffff7f
#define FORWARD_SKIP 0x80
#define REVERSE_SKIP 0xc0
#define DIVIDE_VALUE 8
#define DIVIDE1_VALUE 4
#define DIVIDE2_VALUE 16

#define DISPLAY_LINE_COUNT 18
#define BUFFER_DISPLAY_COUNT 3
#define DISPLAY_BUFFER_SIZE 512  // this needs to be a minumum of BUFFER_DISPLAY_COUNT * 16
#define DISPLAY_UPDATE_TIME 1000 // ms
#define LOG_UPDATE_TIME 30       // sec
#define MIN_SKIP_SIZE 0x1000     // 2MiB  not used?
#define MAX_SKIP_SIZE 0x4000000  // 67,108,864 32GiB
#define DEFAULT_SKIP_SIZE 0x1000 // 4096 2MiB
#define DEFAULT_CLUSTER_SIZE 128
#define DEFAULT_BLOCK_SIZE 1
#define SKIP_MULTI1 750
#define SKIP_MULTI2 1250
#define SKIP_RUN_COUNT 5
#define SKIP_RUN_COUNT_FAST 3
#define SKIP_RUN_RETARD_COUNT 0x1f
#define SKIP_RUN_RETARD_COUNT_FAST 0xf
#define FORWARD 0
#define REVERSE 1
#define FAKE_BAD 0x00000
#define RATE_SKIP_COUNT 2
#define EXIT_ON_SLOW_COUNT 2
#define DEFAULT_RATE_SKIP 50000       // B/s
#define DEFAULT_SKIP_TIMEOUT 1000     // ms
#define ALTERNATE_SKIP_TIMEOUT 300000 // ms, used for exit on slow option
#define DEFAULT_EXIT_ON_SLOW -1000
#define HEAD_PATTERN_CHECK_COUNT 3
#define HEAD_AVERAGE_COUNT 6
#define SCSI_WRITE_TIMEOUT 2000 // ms

#define DRIVER_FILE_NAME "oscdriver"

extern bool forced_exit_ccc;
void cleanup_ccc(void);

void help_ccc(void);
void version_ccc(void);

extern char *debugfile_ccc;

extern char full_command_line_ccc[1024];
extern int log_rows_ccc;
extern int domain_rows_ccc;
extern int total_lines_ccc;
extern int domain_lines_ccc;
extern long long *lposition_ccc;
extern long long *lsize_ccc;
extern long long *lstatus_ccc;
extern long long *temp_lposition_ccc;
extern long long *temp_lsize_ccc;
extern long long *temp_lstatus_ccc;
extern long long *dposition_ccc;
extern long long *dsize_ccc;
extern long long *dstatus_ccc;
extern long long *temp_dposition_ccc;
extern long long *temp_dsize_ccc;
extern long long *temp_dstatus_ccc;
extern long long current_position_ccc;
extern long long current_status_ccc;
extern int domain_status_ccc;
extern long long total_domain_size_ccc;
extern char current_status_string_ccc[64];
extern bool exportdd_ccc;
extern bool regular_file_ccc;
extern bool no_log_backup_ccc;
extern bool disable_log_backup_ccc;
extern char current_model_ccc[41];
extern char current_serial_ccc[21];
extern bool ata_identify_success_ccc;
extern bool ncq_supported_ccc;            // word 76 bit 8
extern bool rebuild_assist_supported_ccc; // word 78 bit 11
extern bool lba_supported_ccc;
extern bool drive_locked_ccc;
extern bool smart_supported_ccc;
extern bool smart_enabled_ccc;
extern bool scsi_read16_supported_ccc;
extern bool scsi_read12_supported_ccc;
extern bool scsi_read10_supported_ccc;
extern bool scsi_read6_supported_ccc;
extern bool scsi_write16_supported_ccc;
extern bool scsi_write12_supported_ccc;
extern bool scsi_write10_supported_ccc;
extern bool scsi_write6_supported_ccc;
extern bool scsi_write_checked_ccc;
extern long long input_offset_ccc;
extern long long output_offset_ccc;
extern long long read_size_ccc;
extern int error_count_ccc;
extern int nontried_count_ccc;
extern int nontrimmed_count_ccc;
extern int nondivided_count_ccc;
extern int nonscraped_count_ccc;
extern int finished_count_ccc;
extern int bad_head_count_ccc;
extern int other_count_ccc;
extern long long error_size_ccc;
extern long long nontried_size_ccc;
extern long long nontrimmed_size_ccc;
extern long long nondivided_size_ccc;
extern long long nonscraped_size_ccc;
extern long long finished_size_ccc;
extern long long bad_head_size_ccc;
extern long long other_size_ccc;
extern long long total_size_ccc;
extern long long retry_error_size_ccc;
extern int derror_count_ccc;
extern int dnontried_count_ccc;
extern int dnontrimmed_count_ccc;
extern int dnondivided_count_ccc;
extern int dnonscraped_count_ccc;
extern int dfinished_count_ccc;
extern int dother_count_ccc;
extern long long derror_size_ccc;
extern long long dnontried_size_ccc;
extern long long dnontrimmed_size_ccc;
extern long long dnondivided_size_ccc;
extern long long dnonscraped_size_ccc;
extern long long dfinished_size_ccc;
extern long long dother_size_ccc;
extern bool no_phase1_ccc;
extern bool no_phase2_ccc;
extern bool no_phase3_ccc;
extern bool no_phase4_ccc;
extern bool no_trim_ccc;
extern bool no_scrape_ccc;
extern bool no_divide1_ccc;
extern bool do_divide2_ccc;
extern bool read_bad_head_ccc;
extern bool rebuild_assist_ccc;
extern int rebuild_assist_test_ccc;
extern bool mark_bad_head_ccc;
extern bool reverse_ccc;
extern int retries_ccc;
extern int retry_count_ccc;
extern long long retried_lba_count_ccc;
extern bool reset_log_ccc;
extern int sector_size_ccc;
extern int cluster_size_ccc;
extern int original_cluster_size_ccc;
extern int block_size_ccc;
extern int block_offset_ccc;
extern long long start_position_ccc;
extern long long end_position_ccc;
extern long long last_display_update_ccc;
extern long long last_display_status_update_ccc;
extern long long last_log_write_ccc;
extern long long last_domain_write_ccc;
extern bool device_fault_ccc;
extern long long test_skip_mb_ccc;
extern long long min_skip_size_ccc;
extern long long original_min_skip_size_ccc;
extern long long max_skip_size_ccc;
extern long long original_max_skip_size_ccc;
extern long long skip_size_ccc;
extern long long skip_history_ccc[5];
extern int skip_count_ccc;
extern int total_skip_count_ccc;
extern int total_slow_skips_ccc;
extern long long last_skip_position_ccc;
extern long long last_skip_jump_position_ccc;
extern long long last_skip_size_ccc;
extern bool skip_ccc;
extern long long copy_start_time_ccc;
extern long long last_finished_value_ccc;
extern long long starting_finished_value_ccc;
extern bool skip_fast_ccc;
extern long long skip_multiplier_ccc;
extern long long skip_mod_ccc;
extern bool skipping_ccc;
extern long long highest_skip_ccc;
extern int skip_run_count_ccc;
extern int skip_run_limit_ccc;
extern int skip_run_retard_count_ccc;
extern long long skip_position_ccc[256];
extern int total_skip_runs_ccc;
extern int total_skip_resets_ccc;
extern long long total_skip_size_ccc;
extern long long last_total_skip_size_ccc;
extern long long skip_run_start_ccc;
extern int direction_ccc;
extern long long rate_history_ccc[256];
extern long long modified_rate_history_ccc[256];
extern long long rate_modifier_ccc;
extern int rate_count_ccc;
extern int rate_history_count_ccc;
extern int rate_skip_count_ccc;
extern long long rate_skip_ccc;
extern int exit_on_slow_count_ccc;
extern long long exit_on_slow_timer_ccc;
extern long long exit_on_slow_ccc;
extern long long exit_slow_time_ccc;
extern long long skip_timeout_ccc;
extern bool skip_time_triggered_ccc;
extern long long longest_read_time_ccc;
extern long long current_read_time_ccc;
extern long long recent_longest_read_time_ccc;
extern long long max_longest_read_time_ccc;
extern long long read_time_history_ccc[256];
extern int read_time_history_count_ccc;
extern long long max_read_rate_ccc;
extern long long last_read_start_time_ccc;
extern long long last_read_size_ccc;
extern char display_buffer_ccc[DISPLAY_BUFFER_SIZE];
extern bool force_mounted_ccc;
extern bool fill_mark_ccc;
extern bool fill_erase_ccc;
extern bool open_source_ccc;
extern long long total_filled_ccc;
extern long long fill_size_ccc;
extern long long last_filled_value_ccc;
extern bool limit_recovery_ccc;
extern int recovery_past_limit_ccc;
extern long long total_runtime_ccc;
extern bool enable_output_offset_ccc;
extern bool enable_current_position_ccc;
extern bool enable_retry_connecting_ccc;
extern bool enable_scsi_write_ccc;
extern bool enable_logfile_phase_ccc;
extern long long last_end_position_ccc;
extern int driver_readahead_multiplier_ccc;
extern bool enable_output_sector_size_ccc;
extern long long output_sector_size_adjustment_ccc;
extern bool use_physical_sector_size_for_virtual_ccc;
extern long long driver_minimum_cluster_size_ccc;
struct chs_t
{
  long long total_cylinders;
  long long total_heads;
  long long sectors_per_track;
  long long cylinder;
  long long head;
  long long sector;
};
extern struct chs_t chs_ccc;
extern long long *head_map_position_ccc;
extern long long *head_map_size_ccc;
extern long long *head_map_final_ccc;
extern int *head_map_head_ccc;
extern int *head_map_status_ccc;
extern long long *temp_head_map_position_ccc;
extern long long *temp_head_map_size_ccc;
extern long long *temp_head_map_final_ccc;
extern int *temp_head_map_head_ccc;
extern int *temp_head_map_status_ccc;
extern int head_map_rows_ccc;
extern int head_map_line_ccc;

struct identify_flags_t
{
  bool sct_error_recovery_control;    // word 206 bit 3
  bool ncq_supported;                 // word 76 bit 8
  bool rebuild_assist_supported;      // word 78 bit 11
  bool extended_support;              // word 83 bit 10
  bool lba_supported;                 // word 49 bit 9
  bool drive_locked;                  // word 128 bit 2
  bool smart_supported;               // word 82 bit 0
  bool smart_enabled;                 // word 85 bit 0
  bool hpa_supported;                 // word 82 bit 10
  bool dco_supported;                 // word 83 bit 11
  bool trim_supported;                // word 169 bit 0
  bool multiword_dma_mode0_supported; // word 63 bit 0
  bool multiword_dma_mode1_supported; // word 63 bit 1
  bool multiword_dma_mode2_supported; // word 63 bit 2
  bool ultra_dma_mode0_supported;     // word 88 bit 0
  bool ultra_dma_mode1_supported;     // word 88 bit 1
  bool ultra_dma_mode2_supported;     // word 88 bit 2
  bool ultra_dma_mode3_supported;     // word 88 bit 3
  bool ultra_dma_mode4_supported;     // word 88 bit 4
  bool ultra_dma_mode5_supported;     // word 88 bit 5
  bool ultra_dma_mode6_supported;     // word 88 bit 6
  bool multiword_dma_mode0_selected;  // word 63 bit 8
  bool multiword_dma_mode1_selected;  // word 63 bit 9
  bool multiword_dma_mode2_selected;  // word 63 bit 10
  bool ultra_dma_mode0_selected;      // word 88 bit 8
  bool ultra_dma_mode1_selected;      // word 88 bit 9
  bool ultra_dma_mode2_selected;      // word 88 bit 10
  bool ultra_dma_mode3_selected;      // word 88 bit 11
  bool ultra_dma_mode4_selected;      // word 88 bit 12
  bool ultra_dma_mode5_selected;      // word 88 bit 13
  bool ultra_dma_mode6_selected;      // word 88 bit 14
  bool solid_state_drive;             // word 217 bit 0
};
extern struct identify_flags_t identify_flags_ccc;

int do_command_line_ccc(void);

int process_arguments_ccc(void);

int increase_log_memory_ccc(int new_lines);

int increase_domain_memory_ccc(int new_lines);

int check_domain_ccc(void);

int get_log_status_ccc(long long mask);

int get_domain_status_ccc(long long mask);

int upline_ccc(int count);

int delete_line_ccc(int line);

int delete_domain_line_ccc(int line);

int insert_line_ccc(int line, long long position, long long size, long long status);

int insert_domain_line_ccc(int line, long long position, long long size, long long status);

int find_block_ccc(long long position);

int find_domain_block_ccc(long long position);

int find_next_block_ccc(int line, long long status, long long mask);

int find_previous_block_ccc(int line, long long status, long long mask);

int change_chunk_status_ccc(long long position, long long size, long long status, long long mask);

int add_to_domain_ccc(long long position, long long size);

int ata_28_pio_read_ccc(unsigned char *reg);

int clone_forward_ccc(void);

int clone_reverse_ccc(void);

int copy_forward_ccc(int status_type, int status_mask, int new_status_type);

int copy_reverse_ccc(int status_type, int status_mask, int new_status_type);

int trim_forward_ccc(int status_type, int status_mask, int new_status_type);

int trim_reverse_ccc(int status_type, int status_mask, int new_status_type);

int phase1_ccc(void);

int phase2_ccc(void);

int phase3_ccc(void);

int trimming_ccc(void);

int scraping_ccc(void);

int retrying_ccc(void);

int read_chunk_ccc(long long output_position, int size);

int write_chunk_ccc(long long position, int size);

int read_ata_dma_ccc(long long position, int size, int rarc_bit);

int read_ata_pio_ccc(long long position, int size);

int scsi_read_ccc(long long position, int size);

int scsi_write_ccc(long long position, int size);

int scsi_check_read_commands_ccc(void);

long long set_additional_status_ccc(int retstat, int skip_info);

long long get_timing_byte_ccc(void);

int process_chunk_ccc(int new_status_type, int retstat, int rsize, int skip_info);

int process_rebuild_assist_chunk_ccc(int status_type, int new_status_type, int retstat, int rsize, int skip_info);

int process_domain_ccc(long long position, int size, int status, int status_mask);

int domain_size_adjustment_ccc(int original_size, int requested_size);

int process_skip_ccc(void);

int reset_skip_ccc(void);

int reset_rate_ccc(void);

int process_bad_head_ccc(void);

int process_skip_run_ccc(void);

int check_device_ccc(void);

char *host_error_code_ccc(int code);

int do_fill_ccc(int status, long long mask);

long long block_align_ccc(long long value, int offset);

long long block_align_up_ccc(long long value);

int call_command_on_error_ccc(void);

int process_chs_ccc(long long position);

void set_soft_timer_ccc(int status);

int write_logfile_phase_ccc(char *phase);

int driver_clone_forward_ccc(long long start, long long small_end, long long big_end);

int initialize_head_map_memory_ccc(void);

int increase_head_map_memory_ccc(int new_lines);

int rebuild_assist_check_head_ccc(int head);

int rebuild_assist_remove_head_ccc(int head);

int rebuild_assist_disable_head_ccc(int head);

int rebuild_assist_get_next_head_ccc(int head);

int enable_rebuild_assist_ccc(void);

int disable_rebuild_assist_ccc(void);

int rebuild_assist_get_number_of_heads_ccc(void);

int get_asc_from_additional_sense_ccc(int value);

int get_ascq_from_additional_sense_ccc(int value);

int get_flag_data_from_identify_ccc(unsigned int wordnumber, unsigned int lowbit, unsigned int highbit);

int get_highest_multiword_dma_mode_ccc(void);

int get_current_multiword_dma_mode_ccc(void);

int get_highest_ultra_dma_mode_ccc(void);

int get_current_ultra_dma_mode_ccc(void);

void test_skip_ccc(int size);

char *get_smart_attribute_name_ccc(int id);

extern char *install_directory_ccc;
extern bool path_fail_ccc;
extern char start_working_directory_ccc[PATH_MAX];
extern char called_name_ccc[PATH_MAX];
extern char full_exec_path_ccc[PATH_MAX * 2];
extern bool found_exec_ccc;

#endif
